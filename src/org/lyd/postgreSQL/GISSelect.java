...
public class GISSelect {
	...
	public String getTreeContents(int id,int type) throws Exception{
		powerDataBase.getSQL("SELECT * FROM PROPERTY.\"PD_OBJECT_TYPE\" WHERE \"UPBELONG_TYPE\"="+type+"");
		...
		if(typeId==1030101){
			treeDirtoryString.append("{'id':'1010401-"+id+"','name':'标准段','isParent':true,'iconSkin':'tree_1010401'},");
			powerDataBase.getSQL("SELECT \"TYPE_ID\",\"TYPE_NAME\" FROM PROPERTY.\"PD_OBJECT_TYPE\" WHERE \"DOWNBELONG_TYPE\"=1030101 AND \"TYPE_ID\"!=1010401");
			...
			while(rs.next()){
				...
				treeDirtory[i].type=typeId;
				treeDirtoryString.append("{'id':'"+treeDirtory[i].type+"-"+treeDirtory[i].id+"','name':'"+treeDirtory[i].name+"','isParent':"+treeDirtory[i].isParent+",'iconSkin':'tree_"+treeDirtory[i].type+"'}");
				if(i<rowCount-1){
					treeDirtoryString.append(",");
				}
...
//		return treeDirtoryString.toString(); 
	...
	public String getTreeContents(int type) throws Exception{
		...
		while(rs.next()){
			...
			treeDirtory[i].isParent=true;
			treeDirtory[i].type=downBelongType;
			treeDirtoryString.append("{'id':'"+type+"-"+treeDirtory[i].id+"','name':'"+treeDirtory[i].name+"','isParent':"+treeDirtory[i].isParent+",'iconSkin':'tree_"+type+"'}");
			if(i<rowCount-1){
				treeDirtoryString.append(",");
			}
			i++;
